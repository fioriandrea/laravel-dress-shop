...
class CheckoutController extends Controller
{
    public function getCheckout() {
        if (auth()->user()->cartProducts->count() == 0) {
            return redirect()->route('user_error', ['messages' => ['Your cart is empty']]);
        }
        if (auth()->user()->addresses->count() == 0) {
            return redirect()->route('user_error', ['messages' => ['You have no addresses']]);
        }
        if (auth()->user()->paymentMethods->count() == 0) {
            return redirect()->route('user_error', ['messages' => ['You have no payment methods']]);
        }
        ...
        foreach (auth()->user()->cartProducts as $cartProduct) {
            if ($cartProduct->product->status == 'unlisted') {
                return redirect()->route('user_error', ['messages' => ['Product: ' . $cartProduct->product->name . ' is unlisted']]);
            }
